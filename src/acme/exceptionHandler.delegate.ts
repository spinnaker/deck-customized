...
export const EXCEPTION_HANDLER = 'spinnaker.acme.exception.handler';
  ...
  .config(($provide: angular.auto.IProvideService) => {
    $provide.decorator('$exceptionHandler', ($injector: IInjectorService,
      ...
      return (exception: Error, cause: string) => {
        ...
        if (!SETTINGS.alert) {
          console.warn('Custom exception handler is not configured - please update the settings.js file to include alert configuration')
        }

        // Add whatever configuration you need (per environment) in the settings.js file to send a structured alert.
        // At Netflix, we send:
        //  * the version (which is generated by build.gradle),
        //  * the user agent,
        //  * the current user,
        //  * the current URL
